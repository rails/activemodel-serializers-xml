...
class ARXmlSerializationTest < ActiveRecord::TestCase
  def test_should_serialize_default_root
    @xml = ARContact.new.to_xml
    assert_match %r{^<ar-contact>},  @xml
    assert_match %r{</ar-contact>$}, @xml
  end

  def test_should_serialize_default_root_with_namespace
    @xml = ARContact.new.to_xml :namespace=>"http://xml.rubyonrails.org/ar-contact"
    assert_match %r{^<ar-contact xmlns="http://xml\.rubyonrails\.org/ar-contact">},  @xml
    assert_match %r{</ar-contact>$}, @xml
  end

  def test_should_serialize_custom_root
    @xml = ARContact.new.to_xml :root => 'xml_contact'
    assert_match %r{^<xml-contact>},  @xml
    assert_match %r{</xml-contact>$}, @xml
  end

  def test_should_allow_undasherized_tags
    @xml = ARContact.new.to_xml :root => 'xml_contact', :dasherize => false
  ...
  def test_should_allow_camelized_tags
    @xml = ARContact.new.to_xml :root => 'xml_contact', :camelize => true
  ...
  def test_should_allow_skipped_types
    @xml = ARContact.new(:age => 25).to_xml :skip_types => true
    assert %r{<age>25</age>}.match(@xml)
  end

  def test_should_include_yielded_additions
    @xml = ARContact.new.to_xml do |xml|
      xml.creator "David"
    end
  ...
  def test_to_xml_with_block
    ...
    xml = ARContact.new.to_xml(:skip_instruct => true) do |_xml|
      _xml.tag! "arbitrary-element", value
    end
    assert_equal "<ar-contact>", xml.first(12)
    assert xml.include?(%(<arbitrary-element>#{value}</arbitrary-element>))
  end

  def test_should_skip_instruct_for_included_records
    @contact = ARContact.new
    @contact.alternative = ARContact.new(:name => 'Copa Cabana')
